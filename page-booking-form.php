...
if ($availability_entry_dep != null) {
	...
	if ($dep_slot_minutes_number <= "360" || $dep_slot_minutes_number >=  "1320") {

		$added_dep_price = $availability_entry_dep->price_private;

		if ($departure_is_private)
			$total_price += ($added_dep_price + $added_dep_price * 20 / 100);
		else {
			$total_price += (($added_dep_price + $added_dep_price * 20 / 100) * $people);
		}
	} else {
...
if (isset($_GET['ret']) && !empty($_GET['ret'])) {
	...
	if ($availability_entry_ret != null) {
		...
		if ($ret_slot_minutes_number <= "360" || $ret_slot_minutes_number >=  "1320") {

			$added_ret_price = $availability_entry_ret->price_private;

			if ($return_is_private)
				$total_price += ($added_ret_price + $added_ret_price * 20 / 100);
			else {
				$total_price += (($added_ret_price + $added_ret_price * 20 / 100) * $people);
			}
		} else {
			if ($return_is_private)
				$total_price += $availability_entry_ret->price_private;
...
<script>
		function work() {
			...
			function asd() {
				tf_price_array[0] = window.bookingRequest.totalPrice;
				setTimeout(function() {
					tf_price_array[1] = window.bookingRequest.totalPrice;
					asd2();
				}, 100);
			}

			function asd2() {
				if (tf_price_array[0] === tf_price_array[1]) {

					asd();

				} else {

					var price = parseFloat(tf_price_array[1]).toFixed(2);
